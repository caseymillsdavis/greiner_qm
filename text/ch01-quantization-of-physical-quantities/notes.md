...
$$
\vec F = -\operatorname{grad} V = \operatorname{grad}(\vec m\cdot\vec H) \qquad (1.9)
$$
...
| Stern–Gerlach | orientation / angular momentum | beam splits in two ($\vec F = \operatorname{grad}(\vec m\cdot\vec H)$) |
